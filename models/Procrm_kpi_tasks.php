...
class Procrm_kpi_tasks extends App_Model
{
    /**
     * Вывод всех
     * @param $where
    ...
    public function get($where)
    {
        ...
        // Всего
        $selectQuery = 'SELECT COUNT(' . db_prefix() . 'tasks.id) FROM ' . db_prefix() . 'tasks';
        $sQuery = $selectQuery;
        ...
        return [
            $count->{'COUNT(' . db_prefix() . 'tasks.id)'},
            $count_1->{'COUNT(' . db_prefix() . 'tasks.id)'},
            $count_4->{'COUNT(' . db_prefix() . 'tasks.id)'},
            $count_3->{'COUNT(' . db_prefix() . 'tasks.id)'},
            $count_2->{'COUNT(' . db_prefix() . 'tasks.id)'},
            $count_5->{'COUNT(' . db_prefix() . 'tasks.id)'},
        ];
    }
}